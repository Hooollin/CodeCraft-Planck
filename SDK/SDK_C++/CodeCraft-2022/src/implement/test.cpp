...
void Test::TestClientDistribution() {
  InputParser *parser;
...
void Test::TestEdgeDistribution() {
  InputParser *parser;
  parser = new InputParser(model_);
  parser->Parse();
  ...
  pre_deal.Distribute();
  std::vector<std::string> edge_order = pre_deal.GetEdgeOrder();
  ...
  for (int i = 0; i < day; i++) {
    ...
    std::unordered_map<std::string, int> edge_bandwidth;
    auto p = pre_distribution[i];
    EdgeDayDistribution dayDistribution(
        i, edge_hash, client_hash, pre_distribution[i], available_edge_node[i],edge_order);
    dayDistribution.DistributeForMyBest();
    p = dayDistribution.GetDistribution();
    out_parser->SetOutput(i, p);
    for (auto &pp : p) {
      std::cout << "client<" << pp.first << ","
